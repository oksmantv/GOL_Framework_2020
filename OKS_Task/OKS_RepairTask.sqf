...
private ["_Temp","_AllHitPoints","_AllHitPointsName","_AllHitPointsValue","_HitPoints","_List","_DamagePoints","_Count"];
...
{ if (["wheel", _x] call BIS_fnc_inString || ["track", _x] call BIS_fnc_inString) then {_Hit pushBackUnique _X}} foreach _AllHitPointsName;

_HitPart = _Hit call BIS_fnc_selectRandom;
_Veh setHit [_HitPart,1];
_Veh setHit ["motor",1];

waitUntil {sleep 10; _Veh getHit _HitPart <= 0.2 || {Alive _X} count units _Friendlies isEqualTo 0};

	//_Task = [_TaskID, player] call BIS_fnc_taskReal;
	//systemChat str _Task;

if({Alive _X} count units _Friendlies isEqualTo 0 && _TaskBoolean) exitWith { [_TaskID,"Failed"] call BIS_fnc_taskSetState; };

_Veh setHit ["motor",0];
_Friendlies addVehicle _Veh;
{_X forceSpeed 5; _X enableAttack false; [_X] orderGetIn true; _X enableAI "path"; _X setUnitPos "UP"; _X disableAI "TARGET"; _X disableAI "AUTOTARGET"; _X disableAI "FSM"; _X disableAI "WEAPONAIM"; _X disableAI "SUPPRESSION"; _X disableAI "COVER"; _X disableAI "AUTOCOMBAT";} foreach units _Friendlies;
...
waitUntil { sleep 5; {_X in _Veh} count units _Friendlies >= count units _Friendlies || {Alive _X} count units _Friendlies isEqualTo 0 || !Alive _Veh || !CanMove _Veh};
