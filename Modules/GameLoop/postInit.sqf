...
[QGVARMAIN(serverReady), {
	if (GVAR(SafeMode_useType) isEqualTo 0 || !(GVAR(SafeMode_Timer) isEqualTo -1)) then {
		GVAR(PFH_SafeMode_Handle) = [{
			...
			if ((GVAR(SafeMode_useType) != 0) && GVAR(SafeMode_Count) isEqualTo GVAR(SafeMode_Timer)) then {
