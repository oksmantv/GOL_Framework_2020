...
switch (_TypeOfObjective) do {
	...
	case "cache": {
		...
		if(!isNull _Area) then {
			...
			while{true} do {
				...
				_SpawnPos = [_RandomPos, 1, 100, 30, 0, 0, 0] call BIS_fnc_findSafePos;
	...
	case "artillery": {
		...
		private ["_Artillery","_Road","_RandomPos","_Target","_SpawnPos","_Arty"];
		...
		if(!isNil "OKS_ArtyFire") then {
			...
			[_Side,_Arty,_Target,7,300,30] remoteExec ["OKS_ArtyFire",0];
		} else {
