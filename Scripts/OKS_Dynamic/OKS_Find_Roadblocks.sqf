/*
	[Trigger_1,15,EAST,["O_Soldier_F"],"colorOPFOR"] execVM "Scripts\OKS_Dynamic\OKS_Find_Roadblocks.sqf";
	[Trigger_1,15,EAST,["O_Soldier_F"],"colorOPFOR"] spawn OKS_Find_Roadblocks.sqf;
*/

if(!isServer) exitWith {};

Params ["_MainTrigger","_RoadblockCount","_Side","_Units","_SideColor"];
private ["_RandomPos","_Road","_marker","_SideMarker","_typeString","_Repetitions","_Debug_Variable"];
...

{
	...
	waitUntil {
		...
		if(_Debug_Variable) then {
			systemChat format ["Found RoadBlock: %1",(!(isNull _Road) && {_Road Distance _X < 300} count OKS_RoadBlock_Positions < 1 &&(getPos _Road) inArea _MainTrigger)];
		};
