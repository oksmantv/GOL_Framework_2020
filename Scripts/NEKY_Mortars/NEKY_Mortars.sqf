	...
	if (Alive _Mortar) then
	{
		if (_Mortar EmptyPositions "Gunner" > 0) then
		{
			...
			Switch (_Side) do
			{
				...
				default { if(_Debug_Variable) then {SystemChat "Invalid side"}};
			};
...
if ( !(_OffMap) && (isNil "_Unit") ) exitWith { if(_Debug_Variable) then {SystemChat "Error while creating mortar gunner, script ends"}};
...
While {((Alive _Mortar) && (Alive _Unit) && (_Unit in _Mortar)) or (_OffMap)} do
{
	...
	if ((!(_FriendlyNear) && !(Count _Units isEqualTo 0)) or ((_OffMap) or !(_Scanner))) then
	{
		...
		if(_Debug_Variable) then {SystemChat "Friendly too close or no hostiles known to mortar faction, rescanning"};
		sleep 10;
	};
	if ((_OffMap) or !(_Scanner)) exitWith
	{
		if(_Debug_Variable) then {systemChat "Off map mode or not in auto mode, exiting script"}
	};
	DeleteVehicle _Zone;
};
if(_Debug_Variable) then {SystemChat "script ending."};